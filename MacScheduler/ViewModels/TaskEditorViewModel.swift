...
class TaskEditorViewModel: ObservableObject {
    ...
    private func validateOutputPath(_ path: String, label: String) {
        ...
        // Resolve symlinks and normalize path to prevent symlink attacks
    ...
    private static func sanitizeNameField(_ string: String) -> String {
        string.unicodeScalars.filter { scalar in
            ...
            // Block dangerous Unicode bidi/zero-width chars
            guard !dangerousUnicodeScalars.contains(scalar) else { return false }
            // For ASCII range: allow printable (32-126) and tab (9)
            if scalar.isASCII {
                return scalar.value >= 32 || scalar.value == 9
            }
            // Allow non-ASCII unicode (accented chars, CJK, etc.)
            return true
        }.map { String($0) }.joined()
    }
